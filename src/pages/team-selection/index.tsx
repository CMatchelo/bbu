...
function useNextMatchInfo() {
  ...
  const sorted = [...leagueTeams].sort(
    (a, b) =>
      (b.stats[user.currentSeason]?.wins ?? 0) -
      (a.stats[user.currentSeason]?.wins ?? 0),
  );
        ...
        {matchInfo && (
          <div className="flex items-center py-3 px-6 rounded-xl bg-cardbg/75 border border-highlights1/15">
            <div className="flex-1 flex items-center justify-end gap-4">
              ...
              <span className="text-[13px] font-medium text-text1 tracking-wider uppercase">
                {matchInfo.homeUni.id}
              </span>
              <span className="text-[14px] text-text1">
                {matchInfo.homeUni.nickname}
              </span>
            </div>
            <span className="text-[13px] text-text2 w-8 text-center shrink-0">x</span>
            <div className="flex-1 flex items-center justify-start gap-4">
              <span className="text-[14px] text-text1">
                {matchInfo.awayUni.nickname}
              </span>
              <span className="text-[13px] font-medium text-text1 tracking-wider uppercase">
                {matchInfo.awayUni.id}
              </span>
              <span className="text-[11px] text-text2">
                ({matchInfo.awayPos})
              </span>
